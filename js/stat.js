...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    ctx.fillRect(CLOUD_X + GAP + (BAR_WIDTH + GAP) * i, STAT_Y - FONT_GAP - (BAR_HEIGHT * times[i]) / maxTime + GAP, BAR_WIDTH, (BAR_HEIGHT * times[i]) / maxTime);
    ctx.fillStyle = '#000000';
    ctx.fillText(names[i], CLOUD_X + GAP + (BAR_WIDTH + GAP) * i, STAT_Y + 2.5 * FONT_GAP);
    ctx.fillText(Math.floor(times[i]), CLOUD_X + GAP + (BAR_WIDTH + GAP) * i, STAT_Y - (BAR_HEIGHT * times[i]) / maxTime);
  }
};
